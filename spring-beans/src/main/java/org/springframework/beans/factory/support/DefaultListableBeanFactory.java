...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	// 所以 BeanDefinition 按照名字以及对应 BeanDefinition 对应
	...
	// Spring 中按照类型得到组件的一个底层 池
	private final Map<Class<?>, String[]> singletonBeanNamesByType = new ConcurrentHashMap<>(64);

	/** List of bean definition names, in registration order. */
	// 保存所以BeanDefiniton的名字
	...
	private String[] doGetBeanNamesForType(ResolvableType type, boolean includeNonSingletons, boolean allowEagerInit) {
		...
		// Check all bean definitions.(笨重实现)因为Spring没有直接保存class--名字对应信息
		// 通过遍历所以 beanName 找到所以符合类型的 BeanDefinition
			...
			if (!isAlias(beanName)) {	//判断是否别名
				try {
					...
					if (!mbd.isAbstract() && (allowEagerInit ||
						...
						boolean isFactoryBean = isFactoryBean(beanName, mbd);	//是否 FactoryBean
			...
			if (descriptor.usesStandardBeanLookup()) {
				...
				if (dependencyName != null) {
					...
					if (isTypeMatch(dependencyName, type) && isAutowireCandidate(dependencyName, descriptor) &&
						...
						Object dependencyBean = getBean(dependencyName);	//【自动装配获取组件】 统一调用 getBean 获取组件
